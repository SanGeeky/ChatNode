...
function Message(type, message, rinfo) {
	...
	this.typeConnect = function() {
		...
		var _message = util.format('Nueva Conexion de %d', this.rinfo.port);
		clients.push(rinfo);
	...
	this.typeDisconnect = function() {
		...
		var _message = util.format('Desconexion de %d', this.rinfo.port);
...
var server = dgram.createSocket('udp4', function(data, rinfo) {
	...
	process.stdin.on('data', function(chunk) {
	 	...
	 	var buffer = new Buffer('Server => %s', chunk.toString().replace(/\n|\n/g, ''));
...
////Aqui enviamos el mensaje a todos los clientes
	});

});

server.bind(port, function() {
	console.log('Server conectado por el puerto %d.', port);
});
